...
const Page = () => {
    ...
    function createRoomName(room: RoomResponse): string {
        let roomNum: string = `${room.building}-${room.roomNum}`;
        if (room.roomName) roomNum = `${roomNum} (${room.roomName})`;
        if (room.subRoom) roomNum = `${roomNum} sala ${room.subRoom}`;
        return roomNum;
    }
            ...
            <div className="flex flex-col justify-start p-3">
                <div className="w-full mt-4">
                    ...
                    <div className="flex justify-between my-1.5">
                        ...
                        <button
                            id="salones-libres"
                            className={`p-2 bg-lime-500 dark:bg-teal-950 ${selectedOptions.includes("salones-libres")
                                    ? "opacity-100"
                                    : "opacity-70"
                                } border-2 rounded-md`}
                        ...
                        <button
                            id="salones-ocupados"
                            className={`p-2 bg-lime-500 dark:bg-teal-950 ${selectedOptions.includes("salones-ocupados")
                                    ? "opacity-100"
                                    : "opacity-70"
                        ...
                        <div className="container px-5 py-10 lg:py-24 mx-auto">
                            <div className="flex flex-wrap -m-4">
                                {rooms.map((room) => (
                                    ...
                                    >
                                        <RoomCard
                                            blockNumber={`Bloque ${room.building}`}
                                            roomNumber={createRoomName(room)}
                                        />
                                    </div>
                                ))}
                            </div>
